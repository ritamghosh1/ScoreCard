...
import { API } from '../App'; // Assuming this is your Axios instance
...
export default function MatchEntry() {
    ...
    const handleSubmit = async (e) => {
        ...
        // 1. Package the data exactly how the backend expects it
        const p1Innings = [{ runs: formData.p1i1Runs, balls: formData.p1i1Balls }];
        const p2Innings = [{ runs: formData.p2i1Runs, balls: formData.p2i1Balls }];

        if (format === 'Test') {
            p1Innings.push({ runs: formData.p1i2Runs, balls: formData.p1i2Balls });
            p2Innings.push({ runs: formData.p2i2Runs, balls: formData.p2i2Balls });
        }

        try {
            // 2. Toss it over the fence to the backend using API.post!
            await API.post('/api/matches', {
                format,
                player1: { name: "Ritam", innings: p1Innings },
                player2: { name: "Riddhiman", innings: p2Innings }
            });
            
            // 3. Delay reset to allow the "Power Shot" animation to finish
            setTimeout(() => {
                setIsSubmitting(false);
                setFormData({ p1i1Runs: '', p1i1Balls: '', p1i2Runs: '', p1i2Balls: '', p2i1Runs: '', p2i1Balls: '', p2i2Runs: '', p2i2Balls: '' });
                alert("Match Saved!"); // Optional: Let the user know it worked!
            }, 2000);
...
// Ensure InningInput is outside the main render but inside the file!
function InningInput({ label, val, setter, border, animate }) {
    ...
    return (
        <motion.div {...props}>
            ...
            <input
                type="number" required={!animate} // Only require Inn 1
